...
Segue o percurso com o dedo: começa no oval, passa pelos dois paralelogramos de entrada e saída, faz as duas contas nos retângulos, mostra os dois resultados e termina. Uma única linha, sem bifurcações, e é isso que significa uma estrutura **sequencial**.
...
| --- | ---: | ---: | ---: | --- |
| Antes de começar | sem valor | sem valor | sem valor | |
| LER cadernos | 30 | sem valor | sem valor | |
| caixasCompletas ← 30 DIV 12 | 30 | 2 | sem valor | |
...
A linha de `caixasAEncomendar` é a que interessa. Pensa: se sobrar alguma unidade, chega encomendar as caixas completas? Escreve por palavras tuas o que tem de acontecer, antes de tentares escrevê-lo em pseudocódigo. Se não conseguires exprimi-lo sem usar a palavra "se", deixa-o em português, porque as decisões são o bloco seguinte.
...
**3. Desenhar o fluxograma (15 min).** Abre a aplicação de diagramas e desenha o fluxograma do algoritmo que completaste. Usa o oval para início e fim, paralelogramos para o `LER` e os `ESCREVER`, e retângulos para as contas. Guarda o ficheiro, que faz parte da evidência deste bloco.
